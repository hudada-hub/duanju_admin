...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isImage && !isVideo && !isAudio && !isDocument) {
      return ResponseUtil.error('不支持的文件类型，请上传 JPG、PNG、GIF、WebP 格式的图片，MP4、WebM、OGG 格式的视频，或 MP3、WAV、OGG、M4A 格式的音频，或 PDF、DOC、XLS、PPT、TXT、ZIP、RAR 格式的文档');
    }
    ...
    // 生成新的文件名
    const ext = file.type.split('/')[1] || (isImage ? 'jpg' : isVideo ? 'mp4' : isDocument ? 'pdf' : 'doc');
